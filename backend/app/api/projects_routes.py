...
def get_categories():
    ...
    for category in data:
      category["length"] = len(Project.query.filter(
          Project.category_id == category["id"]).all())
...
def load_user():
  ...
  categories = Category.query.all()
  pledges = Pledge.query.filter(Pledge.backer_id == user_id).with_entities(
      Pledge.project_id).distinct()
  projects = [Project.query.filter(
      Project.id == pledge.project_id).one() for pledge in pledges]
  data = [category.to_dict() for category in categories]
...
def get_by_id_and_category():
    ...
    pledges = Pledge.query.filter(Pledge.backer_id == user_id).with_entities(
        Pledge.project_id).distinct()
    projects_data = [Project.query.filter(
        Project.id == pledge.project_id, Project.category_id == category_id).first() for pledge in pledges]
    projects = []
...
def submit_pledge():
  ...
  try:
      ...
      db.session.commit()
      pledges = Pledge.query.filter(
          Pledge.backer_id == backer_id, Pledge.project_id == project_id).all()
      project = Project.query.get(project_id)
      if len(pledges) < 2:
        project.increase_pledge()
      reward = Reward.query.get(reward_id)
      pledgeRewards = Pledge.query.filter(
          Pledge.backer_id == backer_id, Pledge.reward_id == reward_id).all()
      ...
      rewards = Reward.query.filter(
          Reward.project_id == project_id).order_by("id").all()
      rewards = [reward.to_dict() for reward in rewards]
      project = project.to_dict()
      return {"project": project, "rewards": rewards}, 200
  except:
